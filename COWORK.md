...
| 2026-08-20 | Homepage video + variant switcher | Two video homepage variants that actually play (full strength and under a 45% black tint), reels staged into a git-ignored `public/videos` with faststart + poster frames; variant switcher rebuilt (labelled control, grouped options with hints, groups disabled with a reason, Reset); `homepage-video-finding.md` — both reels carry burned-in titles so nothing ships on this footage; PLAN §1 rows 40–42 |
| 2026-08-20 | Review of both lanes, then fix | Review of Grok + Cursor against their work orders (1 blocker, 6 defects); then the fixes: all 73 legacy URLs resolve on production's exact targets, 887 photographs render via a git-ignored symlink + `next/image`, trailing slash restored per row 3, alt-text gate added, two unrunnable QA scripts replaced; PLAN §1 rows 33–39; `handoff-to-cursor-2.md` |
