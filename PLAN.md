...
(labeled V — verified, with evidence). Rows 23–42 are from the 20 Aug source-ingest, work-order
and fix sessions (labeled S — the WordPress export arriving in the repo, what it proved, how the
two agent lanes are scoped, what the source files can and cannot supply, and what the review's
findings forced).
...
| 33 S | The 25 REVIEW legacy URLs: honest 404 vs a target | **Adopt the measured target.** All 25 rows (16 unique paths) 301 to a real live project on production today; a 404 would break URLs that work. Targets recorded in `content/redirect-decisions.tsv`, each dated `measured-from-live 2026-08-20`, validated at build time against the live path set. Supersedes "a human target or an honest 404" | The ambiguity was real — each old slug has two claimants — but production already disambiguates, and WordPress's `_wp_old_slug` reproduces it offline. A 404 is not one of the three outcomes PLAN §9 allows (200 / 301 / intentional 410). Review `claude-2026-08-20` §1b |
| 34 S | Cross-category legacy paths | **301 every live slug's other-category path to its canonical path** (58 rules, generated). WordPress resolves an old slug ignoring the category segment, so `/residential/mackage-soho/` serves the commercial project today; exact-path matching alone 404s it. Covers the 7 SKIP rows whose old path carries the other category | Measured: 7 of the 11 SKIP rows return 301 on production, not 200. Without this, 7 working URLs die at cutover |
| 35 S | The SKIP gate's wording | **Restate it.** Old: "the 11 SKIP rows still serving 200" — false for 7 of 11. New: **each of the 11 SKIP slugs still terminates at its own live project, directly or via one 301, and is never redirected to a different project.** Enforced by `scripts/check-redirects.mjs` | The letter of the old gate fails while its intent holds. Checked literally it produces a false failure; checked loosely it hid the 404s in row 33 |
| 36 S | Field gates vs the conflict rule | **A field with an open `source-conflicts.md` §A row renders nothing and is not counted against its gate.** "`sizeSqFt` numeric on all 58" becomes "numeric on all 58 less the slugs with an open §A row"; the suppressed count is reported at migration, not inferred. `lib/skipFields.ts` already implements this across 12 projects and 5 fields | Two gates demanded what CLAUDE.md's conflict rule forbids. Verified: the six open size conflicts (A-1, A-5, A-10, A-11, A-12, A-14) render no value at all — no guess, no placeholder, no zero |
| 37 S | Trailing slash | **Keep it. `trailingSlash: true`.** `/commercial|residential/<slug>/` is canonical, the unslashed form 308s onto it, sitemap and redirect targets are emitted slashed so a legacy hit costs one hop. Honours row 3 literally rather than superseding it | Row 3 says "preserve exactly" and WordPress served the slashed form for four years. The build had been collapsing it to unslashed and declaring that in the sitemap — a silent canonical change nobody decided. Note: the optimizer endpoint becomes `/_next/image/?url=` |
| 38 S | How photography reaches the build | **`public/photos` symlinks the git-ignored dump; `next/image` resizes on demand.** No derivative and no binary is ever committed (`scripts/link-photos.mjs`, gitignored link). `lib/photos.ts` joins gallery order (`inputs/derived/project-images.tsv`) to Cursor's measured originals (`content/images/image-manifest.tsv`). WebP only — AVIF encoded fine but decoded to a blank frame in the review browser. Heroes are `priority`; index tiles stay lazy | 887 sources are 3.12 GB against a 100 MB file cap, so copying was never an option and neither was the legacy CDN. A client picking a design direction must not be the one to discover a decoder bug |
| 39 S | Alt text while it is unwritten | **Photographs render `alt=""` inside a figure labelled by the project title, and `npm run check:alt` reports coverage (0/887 today).** `--strict` fails, and that is the migration gate. Alt is read from `content/images/alt-text.tsv` the moment it exists | Writing 887 descriptions nobody has looked at is invented content (DESIGN §9); filename-as-alt is banned outright. An empty alt on an image whose accessible name comes from adjacent text is correct, and the counter keeps the debt visible instead of silent |

| 40 S | Homepage video variants | **Five homepage variants, not four: `still` · `stills` · `video-loop` (full strength) · `video-tint` (45% black over the footage) · `video-scroll`.** The tint is the addition — the footage is bright and busy, the sidebar is not, and muting it is a real choice about how loud the motion should be. Reels are staged into the git-ignored `public/videos` by `scripts/prepare-videos.mjs` (stream copy + faststart + a real poster frame, no re-encode); playback is muted, looped, `playsInline`, and held on the poster under `prefers-reduced-motion`. With no reel staged the variants render the empty dark field, never a placeholder | Extends PLAN §5's four. Verified playing at 1440: `readyState 4`, muted, looping. **But see row 41 — nothing ships on this footage** |
| 41 S | What the reels can decide | **They answer "should the homepage move", not "which reel ships".** Both carry burned-in marketing typography across the lower third plus a STREAMLINE logo bug, for most of their length — not just an end card — at 1024×576 with a dead audio track. A hero that carries its own headline and logo fights a layout whose whole idea is a wordmark in a 200px sidebar over silent photography. Tinting a title card yields a dimmer title card. Until Eric supplies untitled, ≥1920, cleanly-looping segments, the two stills variants are the only shippable homepage options | `content/findings/homepage-video-finding.md`. Sharpens PLAN §12's "video files without end logos" into a specific ask: no titles, no logo bug, no end card, ≥1920, loops, audio not needed, and say which reel is the homepage |
| 42 S | The variant switcher | **A labelled control, not a word in the footer.** "Review options" reads as a button, shows the current selection under it, and opens a 320px panel over the content with grouped options, a one-line hint each, a radio marker, Esc to close, and **Reset to defaults**. A group that cannot take effect is disabled with the reason — the Homepage group under direction C says "not used by Archive" — and a variant that would render empty is greyed with "no reel staged". Server-resolved capabilities (`photos`, `video`) are threaded through `ReviewProvider` | It read as broken for three separate reasons, all real: it did not look like a control; the Homepage group is genuinely inert under Archive, which renders an index and no hero; and picking a video variant produced a blank screen. State persists in `localStorage`, so a reviewer who landed on an empty variant had no way back — hence Reset |
